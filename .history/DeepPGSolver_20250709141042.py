...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    ...
    def generate_1d_trajectory(self, plot_label="X", Title=""):
        ...
        for i in range(self.D):
            plt.plot(np.linspace(0,self.T, len(traj[:,i,:]) ), traj[:,i,:], linestyle[i], color=str(0), label="Player " + str(i+1))
            plt.plot([self.T], [self.terminal[i].detach().cpu().numpy()], "x", markersize=10, color=str(0.5) )
            plt.plot([0], [self.origin[i].detach().cpu().numpy()], "o", markersize=10, color=str(0.5) )
        plt.grid("True")
        plt.legend()
        plt.title(Title)
        plt.show()
    ...
    def generate_animation(self, filename=None, save=True):
        ...
        if self.state_dim == 2:
            ...
            if save: ani.save("video/trajectory" + filename + ".mp4", writer='ffmpeg', fps=30)
                        # ax_traj.plot(xT, yT, marker='x', markersize=10, color=cmap(1.0), label="Player " + str(i + 1))
            plt.show()
