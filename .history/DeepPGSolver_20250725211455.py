...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    ...
    def train_players(self):
        ...
        if self.Q_type == "leader_follower":
            ...
            leader_net = copy.deepcopy(self.params_net)
            ...
            net_control =   [Network(leader_net) ] + [Network(params_net) for _ in range(1, self.D)]

        else:
            net_control = [Network(params_net) for _ in range(self.D)]  #u=(u1, u2, ..., uD)
