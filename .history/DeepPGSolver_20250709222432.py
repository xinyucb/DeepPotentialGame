...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    ...
    def generate_1d_trajectory(self, plot_label="X", Title=""):
        ...
        for i in range(self.D):
            ...
            plt.plot([0], [self.origin[i].detach().cpu().numpy()], "o", markersize=10, color="red" )
        plt.grid("True")
        plt.legend()
        plt.title(Title)
        plt.show()
    ...
    def generate_2d_trajectory(self, X_trajectory=None,
        ...
        def plot_traj(ax, x, y, color, i, marker_='s', label=True, linestyle="-"):
            T = len(x)
            ax.plot(x, y, linestyle, color=color, linewidth=2)

            # Mark 3 points at 1/4, 1/2, 3/4 and label them
            idxs = [0, T // 4, T // 2, 3 * T // 4, T-1]
            markers = ['o', 's', 's', 's', 'x']
            for j, (idx, marker) in enumerate(zip(idxs, markers)):
                if j >0 and j  < len(idxs)-1:
                    ax.text(x[idx], y[idx], str(j), color=str(1), fontsize=8,
                    ...
                    ax.plot(x[idx], y[idx], marker=marker, markersize=10, color="red", zorder=1 + i)
        ...
        for i in range(self.D):
            ...
            plot_traj(ax_traj, x, y, flat_colors[i % len(flat_colors)], i, linestyle=linestyles[i], marker_=markers[i])
    ...
    def generate_animation(self, filename=None, save=True):
        ...
        if self.state_dim == 2:
            ...
            trajectories = X_trajectory.mean(dim=1).detach().numpy()
            T = len(trajectories)
