...
class deepPG(nn.Module):  # Solver for deep potential game
    ...
    def fetch_minibatch(self):
        ...
        # Modify here if there are some additional randomness invovled
        bernoulli_M = None
    ...
    def loss_function(
        ...
        - bernoulli_M: (M,), additional randomness, (not applicable in the current version)
        Returns:
        ...
        for j in range(0, n):
            ...
            for d in range(self.D):
                if self.control_type == "full":
                    ...
                    Xd = X0[:, d*self.state_dim:(d+1)*self.state_dim]
                    Yd = Y0[:, d*self.state_dim:(d+1)*self.state_dim]
                
                control_weight_list.append(net_actor[d](torch.cat([t0, Xd, Yd], dim=1)))
            
            control = torch.cat(control_weight_list, dim=1)
