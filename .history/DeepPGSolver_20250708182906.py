...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    def __init__(self, net_control, params):
        ...
        self.game_type, self.control_type = self.params_equ["game_type"], self.params_equ["control_type"]
        ...
        self.num_of_orig = self.params_equ["num_of_origin"]
        ...
        self.filename= f"_{self.game_type}_{self.control_type}_D{self.D}_d{self.state_dim}_dest{self.num_of_dest}_orig{self.num_of_orig}_{Q_type}_fiu{model2.fi_param_u}_fix{model2.fi_param_x}_gi{model2.params_equ['gi_param']}_Depth{model2.params_net['depth']}"
    ...
    def partial_xi_fi(self, X0, control, sigma_square=10):
        """
        return:
            - batch size (M) * D  state_dim
        """
        x = X0.view(self.M, self.D, self.state_dim)

        partial_xi_fi = torch.from_numpy(np.zeros([self.M, self.D, self.state_dim]))
        for i in range(self.D):
            x_i = x[:,i].unsqueeze(2)
            x_ = x.permute(0, 2, 1) 

            # Expand Q to (M, D, D)
            Qb = self.Q.unsqueeze(0).expand(self.M, self.D, self.D)
            xi_minus_x = x_i - x_ # batch * state_dim * D
            qi = Qb[:,i,:] #batch * D
            xi_minus_x_square = torch.diagonal(torch.bmm(xi_minus_x.permute(0,2,1), xi_minus_x), dim1=1, dim2=2) #batch * D
            fi =  torch.bmm( xi_minus_x, (- qi *  torch.exp(- xi_minus_x_square * sigma_square)).unsqueeze(-1)).squeeze() #batch*state_dim
            partial_xi_fi[:, i, :]= fi # batch_size * D * state_dim
        return partial_xi_fi '''
    ...
    def f_Phi(self, X0, Y0, control, fi_param_u=0.01, fi_param_x = 10):
        ...
        return (mean_integral * (r_array[1] - r_array[0])).squeeze() * fi_param_x \
            + torch.mean(result_u) * fi_param_u

    '''
    def f_Phi(self, X0, Y0, control, fi_param=1):
        """running cost in the potential function Phi"""
        y = Y0.view(self.M, self.D, self.state_dim) 
        r_array = np.linspace(0,1, 1001)


        integrate_wrt_r = 0
        for r in r_array:
            partial_x_f = self.partial_xi_fi(X0- (1-r)* Y0, control, sigma_square=10) # batch_size * D * state_dim
            result = (partial_x_f * y).sum(dim=[1, 2])  # shape: (batch_size,)
            integrate_wrt_r += torch.mean(result)
         
        integrate_wrt_r *= (r_array[1] - r_array[0])
        return integrate_wrt_r

    


    def f_gaussian_kernel(self, X0, Y0, control, fi_param=1):
        """
        Input:
        - X0, Y0: Batch number (M) * D state_dim TOBEFIXED
        Output:
        - a torch number
        """
        x = X0.view(self.M, self.D, self.state_dim)
        ...
        for i in range(self.D):
            ...
            Qb = self.Q.unsqueeze(0).expand(self.M, self.D, self.D)
            xi_minus_x = x_i - x_
            x_minus_y = x_i - x_ - y_i + y_
            qi = Qb[:,i,:]
            xi_minus_x_square = torch.diagonal(torch.bmm(xi_minus_x.permute(0,2,1), xi_minus_x), dim1=1, dim2=2)
            x_minus_y_square = torch.diagonal(torch.bmm(x_minus_y.permute(0,2,1), xi_minus_x), dim1=1, dim2=2)
            fi =  qi * ( torch.exp(- xi_minus_x_square * 10) - torch.exp(- x_minus_y_square * 10)).squeeze() # M * D
            result_x += torch.mean(fi)

        result_u = control**2 * fi_param
        param = {"aversion": 1, "flocking": -1}
        return result_x * param[self.game_type] * 100 + torch.mean(result_u) * fi_param
    ...
    def simulation_paths(self):
        ...
        X0 = torch.cat([self.Xi.unsqueeze(0).to(self.device)] * self.M).squeeze(2)
        Y0 = torch.cat([self.Yi.unsqueeze(0).to(self.device)] * self.M).squeeze(2)  # batch size * state_di
    ...
    def generate_1d_trajectory(self, plot_label="X"):
        ...
        traj = torch.mean(X_trajectory, dim=1).detach().cpu()
        for i in range(self.D):
            plt.plot(traj[:,i,:])
        plt.show()
        
    def generate_2d_trajectory(self, X_trajectory=None,
                            plot_mean=True, preset_sample_ind=None, plot_label="X", save=False):
    ...
    def generate_animation(self, filename):

        if self.state_dim == 1:
            X_buffer, Y_buffer, loss, net_actor = self.simulation_paths()
            X_trajectory = torch.stack(X_buffer).view(-1, self.M, self.D, self.state_dim)
            traj = torch.mean(X_trajectory, dim=1).detach().cpu().numpy()
            ...
            lines = [ax.plot([], [], color=c, label=f'Player {i}')[0] for i, c in enumerate(colors)]
            ...
            for i in range(4):
                line, = ax.plot([], [], color=colors[i], lw=2, label=f"Player {i}")
            ...
            ani.save("video/trajectory" + filename + ".mp4", writer='ffmpeg', fps=30)
