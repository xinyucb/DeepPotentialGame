...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    ...
    def Actor_step(self, net_actor, optimizer_actor):
        ...
        if self.random_initial_state:
            print("Xi", np.shape(self.Xi))
            Xi = self.Xi +  np.random.rand(self.state_dim * self.D).reshape([-1,1]) * 0.1
            print("Xi perturbed", np.shape(Xi))
            X0 = torch.cat([Xi.unsqueeze(0).to(self.device)] * self.M).squeeze(2)
        ...
        print(np.shape(X0), np.shape(Y0), X0, Y0)
