...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
        ...
        for j in range(0, n):
            ...
            for d in range(self.D):
                if self.control_type == "full":
                    ...
                    if self.Q_type == "leader_follower" and d==0: 
                        added_dim = 1
                    else:
                        added_dim = 0
                    Xd = X0[:, tmp: tmp + self.state_dim + added_dim]
                    Yd = Y0[:, tmp: tmp + self.state_dim]
                    tmp += self.state_dim + added_dim
    ...
    def train_players(self):
        ...
        if self.Q_type=="leader_follower":
            print("leader inputs", self.leader_net["inputs"])
            print("follower inputs", self.leader_net["inputs"])
