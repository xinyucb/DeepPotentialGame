...
class deepPG(nn.Module):  # Solver for deep potential game
    ...
    def fetch_minibatch(self):
        ...
        if bernoulli_M is not None:
            bernoulli_M_tensor = bernoulli_M.unsqueeze(-1).float().to(self.device)
        else:
            bernoulli_M_tensor = None
        ...
        for i in range(self.D):
            ...
            if self.Q_type == "leader_follower":
                if plot_mean:
                    ...
                    xT = 0.5
                    if bernoulli_M is not None and bernoulli_M[preset_sample_ind] == 0: 
                        yT = 0.5
                    else: 
                        yT = -0.5
            else:
