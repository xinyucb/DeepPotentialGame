...
class deepPG(nn.Module):  # Solver for deep potential game
    def __init__(self, net_control, params):
        ...
        if self.Q_type == "leader_follower":
            self.leader_net = params["net0"] #only used in leader-follower games
    ...
    def fetch_minibatch(self):
        ...
        if self.params_equ["Q_type"] == "leader_follower":
            ...
            bernoulli_M = torch.bernoulli(torch.full((self.M,), 0.5)).unsqueeze(-1).float().to(self.device)  # shape: [500]
    ...
    def loss_function(
        ...
        for j in range(0, n):
            ...
            if t1[0,0] < self.T:
                ...
                reward += self.g_quadratic(control, X0, Y0, bernoulli_M, gi_param=self.params_equ["gi_param"])
    ...
    def train_players(self):
        ...
        if self.Q_type=="leader_follower":
            net_actor = [Network(self.leader_net, penalty=self.params_net["penalty"]).to(self.device)] \
                + [Network(self.params_net, penalty=self.params_net["penalty"]).to(self.device)\
        ...
        for it in range(self.params_train["iteration"]):
            ...
            ## Validation
            _, _, loss, _, _ = self.simulation_paths()
    ...
    def generate_leader_targets(self, bernoulli):
        """
        Leader has a random target to be either ([0.5,0.5] or [0.5, -0.5])
        """
        # Initialize a location tensor of shape [M, state_dim] for player 1
        leader_target = torch.empty(self.M, self.state_dim)
        bernoulli = bernoulli.squeeze()

        if self.state_dim==2:
            # Assign values based on Bernoulli outcome
            leader_target[bernoulli == 0] = torch.tensor([0.5, 0.5])
            leader_target[bernoulli == 1] = torch.tensor([0.5, -0.5])
        else:
            print("Leader_follower currently only support 2d games.")

        return leader_target


    def g_quadratic(self, control, X0, Y0, bernoulli_M= None, gi_param=40):
        """
        - X, Y: Batch_size * D state_dim
        control: num of trajectories * num of players, ut
        ...
        """
        if self.params_equ["Q_type"] == "leader_follower":
            leader=X0[:,:self.state_dim]
            target_positions = leader.unsqueeze(2).repeat(1, 1, self.D)  # shape [3, 2, 4]
            target_positions = target_positions.permute(0, self.state_dim, 1).reshape(-1, self.state_dim * self.D)  # shape [3, 8]

            leader_target = self.generate_leader_targets(bernoulli_M)
            target_positions[:,:self.state_dim]= leader_target
            Ter = target_positions
        else:
            Ter = self.terminal.T.expand(self.M, - 1) # batch * D state_dim
        tmp = X0 - 0.5 * Y0 - Ter
        result_x = torch.bmm((tmp.unsqueeze(1)* gi_param).float(), Y0.unsqueeze(2)).squeeze(1)
        return torch.mean(result_x) 
    ...
    def simulation_paths(self):
        ...
        return X_buffer, Y_buffer, loss, net_actor, bernoulli_M
    ...
    def generate_1d_trajectory(self, X_trajectory=None, plot_mean=True, preset_sample_ind=0, \
        ...
        if X_trajectory == None:
            ...
            X_buffer, Y_buffer, loss, net_actor, __annotations__  = self.simulation_paths()
            if plot_label == "X":
        ...
        plt.show()
        plt.close()  # <-- this closes the current figure
        return preset_sample_ind, X_trajectory
        
        


    def generate_2d_trajectory(self, X_trajectory=None, filename="", bernoulli_M=None,
        ...
        if X_trajectory is None:
            X_buffer, Y_buffer, loss, net_actor, bernoulli_M = self.simulation_paths()
            if plot_label == "X":
                X_trajectory = torch.stack(X_buffer).view(-1, self.M, self.D, state_dim)
            elif plot_label == "Y":
                X_trajectory = torch.stack(Y_buffer).view(-1, self.M, self.D, state_dim)
        ...
        for i in range(self.D):
            ...
            if self.Q_type == "leader_follower":
                if plot_mean:
                    ...
                    xT = 0.5
                    if bernoulli_M[preset_sample_ind] == 0: yT = 0.5
                    else: yT = -0.5
            else:
                xT, yT = self.terminal.view(self.D, -1).detach().numpy()[i]
            x0, y0 = self.origin.view(self.D, -1).detach().numpy()[i]
            plot_traj(ax_traj, x, y, xT, yT, x0, y0, flat_colors[i % len(flat_colors)],\
                       i, linestyle=linestyles[i], marker_=markers[i])
        ...
        return preset_sample_ind, X_trajectory, bernoulli_M
    ...
    def generate_animation(self, X_trajectory=None, plot_mean=True, preset_sample_ind=0, filename=None, save=True):
        ...
        if X_trajectory==None:
            X_buffer, Y_buffer, loss, net_actor, _ = self.simulation_paths()
