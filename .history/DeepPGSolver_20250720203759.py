...
class FBSNN(nn.Module):  # Forward-Backward Stochastic Neural Network
    ...
    def train_players(self):
        ...
        for it in tqdm(range(self.params_train["iteration"])):
            ...
            print('It: %d, loss_training: %.4f, val_loss: %.4f' % (it, self.dict["loss_actor"].item(), loss))
            ...
            stop_training = any(param_group['lr'] < 1e-5 for param_group in optimizer_actor.param_groups)
            if stop_training:
        ...
        for d in range(self.D):
            self.dict[f'net_actor_{d}'] = net_actor[d].state_dict()
        for param_group in optimizer_actor.param_groups:
            self.params_train["lr_actor"] = param_group['lr']
